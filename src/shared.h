...
class stack
{
   public:
	...
	stack( const stack<T>& );
	~stack();
	T& operator[]( int i ) { return this->m_data[i]; }
...
class str
{
	...
	*/
	...
	str& operator=( const str& s ) { return this->operator=( s.data ); }
	str& operator=( const char* s )
	{
		if( s )
		{
	...
	bool operator==( str& s ) { return !strcmp( data, s.data ); }
	bool operator==( const char* pstr ) { return !strcmp( data, pstr ); }
	char* data;
};
