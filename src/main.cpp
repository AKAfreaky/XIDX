...
int usage( int code )
{
	fprintf( stdout,
	    ...
	    "  -c      create. Uses '" DEFAULT_OUTPUT_FNAME
	    "' as output file name by default.\n"
	    "  -x      extract\n"
	    "  -t      list\n"
	    ...
	    "  -b      short for '-B " DEFAULT_MANIP_SUFFIX
	    "' for compatibility with XIDX prior to version 1.5\n"
...
int main( int argc, char* argv[] )
{
	...
	fputs(
	    "idx extractor/packer\nVersion 1.6.2\nMade by The Europa Barbarorum Team, based on the tool by "
	    "Vercingetorix\n\n",
	    stdout );
...
#if PATH_SEP != '\\'
						fputs(
						    "Warning: '-p' is equivalent to '-P' on this OS/platform (but recognised separately for "
						    "consistency)\n",
						    stderr );
#endif
						break;
	...
	switch( function )
	{
		...
		case CREATE:
		{
			...
			if( ofile == NULL )
			{
				...
				fprintf(
				    stderr, "Warning : no output file name specified (use the -f option), will use '%s'\n", ofile );
			}
			if( files.empty() )
			{
...
static xidxFunc functions[] = {xidxFunc( "help:?:man", HELP_HELP_STR, &xidxShell::help ),
    xidxFunc( "exit:quit", "exits xidx", &xidxShell::exit ), xidxFunc( "scale", HELP_SCALE_STR, &xidxShell::scale ),
    // todo://xidxFunc( "skel", HELP_SKEL_STR, &xidxShell::loadskel ),
    xidxFunc( "copy:cp", HELP_COPY_STR, &xidxShell::copy ), xidxFunc( "chpath", HELP_CHPATH_STR, &xidxShell::chpath ),
#ifdef _WIN32
    xidxFunc( "clear:cls:clr", HELP_CLEAR_STR, &xidxShell::clear ),
#endif
    xidxFunc( "mkdir", HELP_MKDIR_STR, &xidxShell::mkdir ), xidxFunc( "fork:~:`", HELP_FORK_STR, &xidxShell::fork )};
...
int xidxShell::readfile( const char* filename, const char* mode, char*& buffer, FILE*& fp, unsigned int& fsize )
{
...
int xidxShell::chpath( int argc, char** argv )
{
	...
	for( int i = 0; i < argc; i++ )
	{
		...
		if( !strcasecmp( argv[i], "all" ) )
			wildcard = true;
		else if( 0 != strcasecmp( argv[i], "list" ) )
			list = true;
		else if( 0 != strcasecmp( argv[i], "copy" ) )
			copy = true;
