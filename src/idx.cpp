...
#define IO_BUF_CHECK( a, b, buf, fp )                                                                                  \
	if( io_buf_check( ( a ), ( b ), buf, fp ) )                                                                        \
	{                                                                                                                  \
		buf = NULL;                                                                                                    \
		return false;                                                                                                  \
	}
...
void idxSubFile::ParseFilename( const char* fname )
{
	...
	else
	{
		if( !formated )
		{
			if( strchr( tstr, '/' ) ) fprintf( stderr, "warning: file %s does not have \"data\" in path\n", tstr );
...
bool idxFile::OpenFiles( const char* filename, const char* mode )
{
	...
	static const int max = MAGIC_LENGTH( DAT_SUFFIX ) > MAGIC_LENGTH( IDX_SUFFIX ) ? MAGIC_LENGTH( DAT_SUFFIX )
	                                                                               : MAGIC_LENGTH( IDX_SUFFIX );
	int s_len = max + len + 1;
	...
	for( unsigned int i = 0; i < numFiles; i++ )
	{
		...
		else
		{
			...
			if( verbose || list )
				...
				fprintf( stderr, "Extracting file %d/%d\r", i, numFiles );
		}
	}
	CloseFiles();
...
bool sidxSubFile::Read( FILE* fp )
{
	...
	/*read filename*/
	for( unsigned int i = 0; ( filename[i ? i - 1 : 0] || !i ) && i < MAGIC_LENGTH( filename ); i++ )
		filename[i] = fgetc( fp );
	fseek( fp, 3, SEEK_CUR );
